...
public final class TestDatabasePreparer {
    ...
    public func save(_ block: () -> Void) {
        objCnx.performAndWait {
            block()
            do {
                try objCnx.obtainPermanentIDs(for: Array(objCnx.insertedObjects))
                try objCnx.save()
            }
            catch {
                print("\(error)")
    ...
    public func saveAsync<T>(_ block: @escaping () throws -> T) async rethrows -> T {
        try await objCnx.perform(schedule: .immediate) {
            let returnValue = try block()
            do {
                try self.objCnx.obtainPermanentIDs(for: Array(self.objCnx.insertedObjects))
    ...
    @discardableResult public func createBallot(
        ...
        ballotID: Data = BytesUtility.generateBallotID()
    ) -> BallotEntity {
    ...
    @discardableResult public func createContact(
        publicKey: Data = BytesUtility.generatePublicKey(),
        identity: String,
    ...
    @discardableResult public func createFileMessageEntity(
        ...
        mimeType: String? = nil,
        type: NSNumber? = nil,
        messageID: Data = BytesUtility.generateMessageID(),
        date: Date = Date(),
        isOwn: Bool = false,
        sent: Bool = true,
    ...
    func createGroup(
        ...
        members: [String],
        myIdentityStoreMock: MyIdentityStoreMock? = nil
    ) throws -> (ContactEntity?, GroupEntity, ConversationEntity) {
        save {
            let isSelfCreator = myIdentityStoreMock?.identity == groupCreatorIdentity
            var creatorContactEntity: ContactEntity?

            if !isSelfCreator {
                creatorContactEntity = loadEntity(
            ...
            let groupEntity = createGroupEntity(
                groupID: groupID,
                groupCreator: isSelfCreator ? nil : groupCreatorIdentity
            )
            let conversation = createConversation()
            conversation.groupID = groupEntity.groupID
            if let creatorContactEntity {
                conversation.contact = creatorContactEntity
            }
            ...
            return (creatorContactEntity, groupEntity, conversation)
        }
    }
