...
public final class EntityDestroyer: NSObject {
    ...
    private let objCnx: ThreemaManagedObjectContext
    private let isRemoteSecretEnabled: Bool
    
    required init(
        managedObjectContext: ThreemaManagedObjectContext,
    ) {
        self.objCnx = managedObjectContext
        self.isRemoteSecretEnabled = managedObjectContext.usesAdditionallyEncryptedModel
    }
    ...
    private func deleteMedias<T: BaseMessageEntity>(of messages: [BaseMessageEntity], messageType: T.Type) throws {
        ...
        if !deleteMediaIDs.isEmpty {
            ...
            if !updateMessageIDs.isEmpty {
                ...
                DatabaseContext.changed(objectIDs: Set(updatedIDs))
    ...
    public func deleteMessageContent(of message: BaseMessageEntity) throws {
                ...
                do {
                    ...
                    if message is FileMessageEntity || message is ImageMessageEntity {
                        self.objCnx.delete(object)
                            ...
                            let defaultThumbnail = UIImage(systemName: "film"),
                            let data = defaultThumbnail.jpegData(compressionQuality: 0.8) {
    ...
    /// - Returns:
    ///    Count of deleted messages
    public func deleteMessages(of conversation: ConversationEntity) -> Int {
        let fetchMessages = NSFetchRequest<NSFetchRequestResult>(entityName: "Message")
        fetchMessages.predicate = NSPredicate(format: "conversation = %@", conversation)
        ...
        if deletedMessages > 0 {
            NotificationCenter.default.post(
                ...
                object: nil,
                userInfo: [DatabaseContext.conversationObjectIDKey: conversation.objectID]
            )
        }
        return deletedMessages
    }

    @objc public func delete(ballot: BallotEntity) {
        delete(entity: ballot)
    }
    
    public func delete(reaction: MessageReactionEntity) {
        delete(entity: reaction)
    }
    
    func delete(ballotChoice: BallotChoiceEntity) {
    ...
    public func delete(conversation: ConversationEntity) {
        ...
        public func delete(contactEntity: ContactEntity) {
    ...
    public func delete(groupCallEntity: GroupCallEntity) {
    ...
    public func delete(imageDataEntity: ImageDataEntity) {
        delete(entity: imageDataEntity)
    }

    public func delete(lastGroupSyncRequestEntity: LastGroupSyncRequestEntity) {
        delete(entity: lastGroupSyncRequestEntity)
    }

    public func delete(messageHistoryEntryEntity: MessageHistoryEntryEntity) {
    ...
    ///
    /// - Parameters:
    ///   - contactEntity: Delete 1:1 conversation of this contact
    ///   - conversationWillBeDeleted: Called before the conversation will be delete
    public func deleteOneToOneConversation(
        for contactEntity: ContactEntity,
        conversationWillBeDeleted: (ConversationEntity) -> Void
    ) {
        if let conversations = contactEntity.conversations {
            for conversation in conversations.filter({ !$0.isGroup }) {
                conversationWillBeDeleted(conversation)
    ...
    ///
    /// - Parameter identity: The identity of the local user
    /// - Returns: True own contact was found and deleted
    public func deleteOwnContact(myIdentity: String?) -> Bool {
        ...
        fetchContacts.predicate = NSPredicate(format: "identity = %@", myIdentity)
    ...
    public func orphanedExternalFiles(appGroupID: String) -> (orphanedFiles: [String], totalFilesCount: Int) {
        
        // Load all external filenames
        if let files = FileUtility.shared
            .dir(
                at: FileUtility.shared.appDataDirectory(appGroupID: appGroupID)?
                    .appendingPathComponent("\(DatabaseManager.databaseExternalStoragePath)/")
            ),
            !files.isEmpty {
            ...
            let orphanedFiles = files.map(\.lastPathComponent)
                .filter { file -> Bool in
    ...
    public func deleteMissedCallsCacheOlderThanTwoWeeks() {
        ...
        do {
            ...
            if let deletedIDs = deleteResult?.result as? [NSManagedObjectID], !deletedIDs.isEmpty {
                DatabaseContext.mergeChangesIntoMainContext([NSDeletedObjectsKey: deletedIDs])
                DatabaseContext.mergeChangesIntoDirectContexts([NSDeletedObjectsKey: deletedIDs])
            }
            else {
    ...
    private func getMediaMetaInfo(
        ...
        if messageType is AudioMessageEntity.Type {
            ...
            relationship = AudioMessageEntity.Field.name(for: .audio, encrypted: isRemoteSecretEnabled)
            blobIDField = AudioMessageEntity.Field.name(for: .audioBlobID, encrypted: isRemoteSecretEnabled)
        }
        else if messageType is FileMessageEntity.Type {
            fetchMessages = NSFetchRequest<NSManagedObject>(entityName: "FileMessage")
            relationship = FileMessageEntity.Field.name(for: .data, encrypted: isRemoteSecretEnabled)
            blobIDField = FileMessageEntity.Field.name(for: .blobID, encrypted: isRemoteSecretEnabled)
        }
        else if messageType is ImageMessageEntity.Type {
            fetchMessages = NSFetchRequest<NSManagedObject>(entityName: "ImageMessage")
            relationship = ImageMessageEntity.Field.name(for: .image, encrypted: isRemoteSecretEnabled)
            blobIDField = ImageMessageEntity.Field.name(for: .imageBlobID, encrypted: isRemoteSecretEnabled)
        }
        else if messageType is VideoMessageEntity.Type {
            fetchMessages = NSFetchRequest<NSManagedObject>(entityName: "VideoMessage")
            relationship = VideoMessageEntity.Field.name(for: .video, encrypted: isRemoteSecretEnabled)
            blobIDField = VideoMessageEntity.Field.name(for: .videoBlobID, encrypted: isRemoteSecretEnabled)
        }
        else {
    ...
    private func getExternalFilenames(ofMessages: [Any], includeThumbnail: Bool) -> [String] {
        ...
        for message in ofMessages {
            guard message is AudioMessageEntity || message is FileMessageEntity || message is ImageMessageEntity ||
                message is VideoMessageEntity else {
                continue
            }
            ...
            if let audioMessage = message as? AudioMessageEntity {
                if let externalFileName = audioMessage.audio?.getFilename() {
                    externalFilenames.append(externalFileName)
                }
            }
            else if let fileMessage = message as? FileMessageEntity {
                if let externalFileName = fileMessage.data?.getFilename() {
                    externalFilenames.append(externalFileName)
                }
                if includeThumbnail, let thumbnailFileName = fileMessage.thumbnail?.getFilename() {
                    externalFilenames.append(thumbnailFileName)
                }
            }
            else if let imageMessage = message as? ImageMessageEntity {
                if let externalFileName = imageMessage.image?.getFilename() {
                    externalFilenames.append(externalFileName)
                }
                if includeThumbnail, let thumbnailFileName = imageMessage.thumbnail?.getFilename() {
                    externalFilenames.append(thumbnailFileName)
                }
            }
            else if let videoMessage = message as? VideoMessageEntity {
                if let externalFileName = videoMessage.video?.getFilename() {
                    externalFilenames.append(externalFileName)
                }
                if includeThumbnail, let thumbnailFileName = videoMessage.thumbnail?.getFilename() {
                    externalFilenames.append(thumbnailFileName)
                }
            }
        }
    ...
    private func deleteExternalFiles(list: [String]) {
        ...
        // If we do that yet Core Data crashes with: "External data reference can't find underlying file."
        // swiftformat:disable:next all
        return;

//        if !list.isEmpty {
//            for filename in list {
//                let fileURL = FileUtility.shared.appDataDirectory(appGroupID: AppGroup.groupID())?
//                    .appendingPathComponent("\(DatabaseManager.databaseExternalStoragePath)/\(filename)")
//                FileUtility.shared.deleteIfExists(at: fileURL)
//            }
//        }
    }
    ...
    private func refreshDatabaseMainAndDirectContexts(for deletedIDs: [NSManagedObjectID]) {
        ...
        DatabaseContext.mergeChangesIntoMainContext(deletedObjects)
        DatabaseContext.mergeChangesIntoDirectContexts(deletedObjects)
    }
}
