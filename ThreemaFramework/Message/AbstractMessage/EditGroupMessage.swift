...
@objc public final class EditGroupMessage: AbstractGroupMessage {
    ...
    override public func body() -> Data? {
        guard let groupCreatorASCII = groupCreator?.data(using: .ascii),
            ...
            DDLogError("Unable to create EditGroupMessage body")
            return nil
        }
    ...
    @objc func fromRawProtoBufMessage(rawProtobufMessage: NSData) throws {
        decoded = try CspE2e_EditMessage(serializedBytes: rawProtobufMessage as Data)
    }
    ...
    required init?(coder: NSCoder) {
        ...
        do {
            ...
            self.decoded = try CspE2e_EditMessage(serializedBytes: Data(data))
        }
        catch {
