...
- (void)fetchIdentityInfo:(NSString*)identity onCompletion:(void(^)(NSData *publicKey, NSNumber *state, NSNumber *type, NSNumber *featureMask))onCompletion onError:(void(^)(NSError *error))onError {
    ...
    [ServerAPIRequest loadJSONFromAPIPath:apiPath withCachePolicy:NSURLRequestReloadIgnoringLocalCacheData onCompletion:^(id jsonObject) {
        ...
        NSNumber *featureMask = @0;
        if (![jsonObject[@"featureMask"] isEqual:[NSNull null]]) {
            featureMask = [NSNumber numberWithInt:[jsonObject[@"featureMask"] intValue]];
        }
...
- (void)fetchBulkIdentityInfo:(NSArray*)identities onCompletion:(void(^)(NSArray *identities, NSArray *publicKeys, NSArray *featureMasks, NSArray *states, NSArray *types))onCompletion onError:(void(^)(NSError *error))onError {
    ...
    [ServerAPIRequest postJSONToAPIPath:@"identity/fetch_bulk" data:req onCompletion:^(id jsonObject) {
        ...
        while(index < count) {
            ...
            if ([item[@"featureMask"] isEqual: [NSNull null]]) {
                [responseFeatureMasks addObject:@0];
            } else {
...
- (void)checkRevocationPasswordForStore:(id<MyIdentityStoreProtocol>)identityStore onCompletion:(void(^)(BOOL revocationPasswordSet, NSDate *lastChanged))onCompletion onError:(void(^)(NSError *error))onError {
    ...
    [self sendSignedRequest:request toApiPath:@"identity/check_revocation_key" forStore:identityStore onCompletion:^(NSDictionary *response) {
        ...
        if (revocationPasswordSet)
            lastChanged = [ThreemaUtilityObjC parseISO8601DateString:response[@"lastChanged"]];
...
- (void)obtainAuthTokenOnCompletion:(void(^)(NSString *authToken))onCompletion onError:(void(^)(NSError *error))onError {
    [ServerAPIRequest loadJSONFromAPIPath:@"auth_token" withCachePolicy:NSURLRequestReloadIgnoringCacheData onCompletion:^(id jsonObject) {
        if (jsonObject[@"authToken"]) {
            onCompletion(jsonObject[@"authToken"]);
        } else {
            onError([ThreemaError threemaError:jsonObject[@"error"]]);
        }
    } onError:onError];
