...
NSString * const MDM_KEY_DISABLE_IOS_SYSTEM_BACKUPS_ID_KEY_INCLUSION = @"th_disable_ios_system_backups_id_key_inclusion"; // Bool
...
- (BOOL)disableIOSSystemBackupsIDKeyInclusion {
    NSNumber *disableIOSSystemBackupsIDKeyInclusion = [self getMdmConfigurationBoolForKey:MDM_KEY_DISABLE_IOS_SYSTEM_BACKUPS_ID_KEY_INCLUSION];
    return [disableIOSSystemBackupsIDKeyInclusion isKindOfClass:[NSNumber class]] ? disableIOSSystemBackupsIDKeyInclusion.boolValue : NO;
}
...
- (void)loadIDCreationValues {
    ...
    if ([self existsMdmKey:MDM_KEY_FIRST_NAME]) {
        ...
        if ([firstName isKindOfClass:[NSString class]]) {
            ...
            DDLogError(@"[MDMSetup] loadIDCreationValues firstName is not a string");
    ...
    if ([self existsMdmKey:MDM_KEY_LAST_NAME]) {
        ...
        if ([lastName isKindOfClass:[NSString class]]) {
            ...
            DDLogError(@"[MDMSetup] loadIDCreationValues lastName is not a string");
    ...
    if ([self existsMdmKey:MDM_KEY_CSI]) {
        ...
        if ([csi isKindOfClass:[NSString class]]) {
            ...
            DDLogError(@"[MDMSetup] loadIDCreationValues csi is not a string");
    ...
    if ([self existsMdmKey:MDM_KEY_JOB_TITLE]) {
        ...
        if ([jobTitle isKindOfClass:[NSString class]]) {
            ...
            DDLogError(@"[MDMSetup] loadIDCreationValues jobTitle is not a string");
    ...
    if ([self existsMdmKey:MDM_KEY_DEPARTMENT]) {
        ...
        if ([department isKindOfClass:[NSString class]]) {
            ...
            DDLogError(@"[MDMSetup] loadIDCreationValues department is not a string");
    ...
    if ([self existsMdmKey:MDM_KEY_CATEGORY]) {
        ...
        if ([category isKindOfClass:[NSString class]]) {
            ...
            DDLogError(@"[MDMSetup] loadIDCreationValues category is not a string");
        ...
        if (isBusinessApp) {
            if (!isSetup) {
                ...
                if (_mdmCacheSetup == nil) {
                    ...
                    if ([_mdmCacheSetup valueForKey:MDM_KEY_SAFE_PASSWORD] != nil) {
                        [[NSNotificationCenter defaultCenter] postNotificationName:kSafeBackupPasswordCheck object:nil userInfo:nil];
                    }
                }
                mdm = _mdmCacheSetup;
...
- (void)printMDMIntoDebugLog:(NSDictionary *)mdm {
    if ([[mdm allKeys] containsObject:MDM_KEY_THREEMA_PARAMS]) {
        ...
        for(NSString *key in mdm[MDM_KEY_THREEMA_PARAMS]) {
            [self checkIsPersonalInfoAndLog:key value:[mdm[MDM_KEY_THREEMA_PARAMS] objectForKey:key]];
        }
    } else {
        for(NSString *key in mdm) {
            [self checkIsPersonalInfoAndLog:key value:[mdm objectForKey:key]];
        }
    }
}
    ...
    if ([key containsString:@"pass"] && ![key isEqualToString:MDM_KEY_SAFE_PASSWORD_PATTERN] && ![key isEqualToString:MDM_KEY_SAFE_PASSWORD_MESSAGE] && value != nil) {
        DDLogNotice(@"[MDMSetup] %@: ***", key);
            ...
            if ([value length] == 0) {
                ...
                DDLogNotice(@"[MDMSetup] %@: %@***", key, [value substringToIndex:value.length - 1]);
            }
            else {
                DDLogNotice(@"[MDMSetup] %@: %@***", key, [value substringToIndex:2]);
            }
        }
    else {
