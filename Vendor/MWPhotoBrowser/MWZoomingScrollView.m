...
- (void)setPhoto:(id<MWPhoto>)photo {
    ...
    _photoImageView.accessibilityLabel = [_photo accessibilityLabelForContent];
...
//        }
        ...
        /***** BEGIN THREEMA MODIFICATION: zoom fix for iphone x *********/
        if (imageSize.width * zoomScale - boundsSize.width > 1) {
            zoomScale = xScale;
        }
        /***** END THREEMA MODIFICATION: zoom fix for iphone x *********/
    }
    return zoomScale;
}
...
- (void)handleSingleTap:(CGPoint)touchPoint {
    ...
    if ([_photo respondsToSelector:@selector(handleSingleTap:)]) {
        [_photo handleSingleTap:touchPoint];
    } else {
