...
public class DKGroupDataManager: DKBaseManager, PHPhotoLibraryChangeObserver {
	...
	public func fetchGroups(_ completeBlock: @escaping (_ groups: [String]?, _ error: NSError?) -> Void) {
		if let assetGroupTypes = self.assetGroupTypes {
			DispatchQueue.global(qos: .userInteractive).async {
                ...
                let fetchMoments = PHAssetCollection.fetchMoments(with: nil)
                if (fetchMoments.count > 0) {
                    var assetsCount = 0
                    fetchMoments.enumerateObjects({ (collection, idx, stop) in
                        assetsCount = assetsCount + collection.estimatedAssetCount
                    })
                    
                    let momentsAssets = [PHAsset](repeating: PHAsset(), count: assetsCount)
                    let collection = PHAssetCollection.transientAssetCollection(with: momentsAssets, title: NSLocalizedString("gallery_moments", comment: ""))
                    let assetGroup = strongSelf.makeDKAssetGroup(with: collection)
                    assetGroup.momentsResult = fetchMoments
                    if strongSelf.showsEmptyAlbums || assetGroup.totalCount > 0 {
                        groups[assetGroup.groupId] = assetGroup
                        groupIds.insert(assetGroup.groupId, at: 1)
                    }
                    if !groupIds.isEmpty {
                        strongSelf.updatePartial(groups: groups, groupIds: groupIds, completeBlock: completeBlock)
                    }
	...
	public func fetchGroupThumbnailForGroup(_ groupId: String, size: CGSize, options: PHImageRequestOptions, completeBlock: @escaping (_ image: UIImage?, _ info: [AnyHashable: Any]?) -> Void) {
        ...
        /***** BEGIN THREEMA MODIFICATION: add reload data *********/
        let latestAsset: DKAsset!
        if (group.momentsResult != nil && !group.momentsLoaded) {
            
            let result = PHAsset.fetchAssets(in: (group.momentsResult?.lastObject)!, options: nil)
            latestAsset = DKAsset(originalAsset: result.lastObject!)
            latestAsset.fetchImageWithSize(size, options: options, completeBlock: completeBlock)
        } else {
            latestAsset = DKAsset(originalAsset:group.fetchResult.lastObject!)
            latestAsset.fetchImageWithSize(size, options: options, completeBlock: completeBlock)
        }
        /***** END THREEMA MODIFICATION: add reload data *********/
	}
    ...
    /***** BEGIN THREEMA MODIFICATION: add reload data *********/
    public func loadMomentsGroup(_ group: DKAssetGroup, completion: () -> ()) {
        
        var momentsAssets = [PHAsset]()
        
        group.momentsResult?.enumerateObjects({ (collection, idx, stop) in
            let fetchResult = PHAsset.fetchAssets(in: collection, options: nil)
            fetchResult.enumerateObjects({ (assets, jdx, stop2) in
                momentsAssets.append(assets)
            })
        })
        let collection = PHAssetCollection.transientAssetCollection(with: momentsAssets, title: NSLocalizedString("gallery_moments", comment: ""))
        self.updateGroup(group, collection: collection)
	...
	public func photoLibraryDidChange(_ changeInstance: PHChange) {
        ...
        /***** BEGIN THREEMA MODIFICATION: check self.groups not nil *********/
        guard self.groups != nil else {
            return
        }
        /***** END THREEMA MODIFICATION: check self.groups not nil *********/
