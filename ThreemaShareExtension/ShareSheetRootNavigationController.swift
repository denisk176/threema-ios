...
class ShareSheetRootNavigationController: UINavigationController {
    ...
    private var connectionStateResolver: Resolver<Void>?
    ...
    deinit {
        // Unregister connection state delegate
    ...
    private func setContactsFromIntent() {
        guard let intent = extensionContext?.intent as? INSendMessageIntent,
              ...
              let managedObject = businessInjector.entityManager.entityFetcher
              .existingObject(with: selectedIdentity) else {
            return
        }
    ...
    private func startExtension() {
        if !businessInjector.userSettings.ipcCommunicationEnabled {
            businessInjector.serverConnector.connect(initiator: .shareExtension)
        }
    ...
    // Implemented with a completion handler because the entry point of the share extension isn't async
    private func extensionIsReady(completion: @escaping (Bool) -> Void) {
        ...
        Task {
            do {
                ...
                let cachedViewControllers = self.viewControllers
                ...
                self.viewControllers = cachedViewControllers
                ...
                DDLogWarn("[ShareExtension] Business injector is not ready to use")
    ...
    /// Connect to server and wait until connection state is loggedIn or disconnected.
    /// Thorws the error `ThreemaProtocolError.notLoggedIn` after a timeout of 20 seconds.
    /// - Throws: ThreemaProtocolError.notLoggedIn
    private func connectToServer() -> Promise<Void> {
        let connect = Promise<Void> { seal in
            if businessInjector.serverConnector.connectionState == .loggedIn {
                seal.fulfill_()
                return
            }
            ...
            self.connectionStateResolver = seal

            businessInjector.serverConnector.connect(initiator: .shareExtension)
        }

        return race(when(fulfilled: [connect]), connectTimeout(seconds: 20)).then { () -> Promise<Void> in
            guard self.businessInjector.serverConnector.connectionState == .loggedIn else {
    ...
    private func connectTimeout(seconds: Int) -> Promise<Void> {
        Promise { seal in
            after(seconds: TimeInterval(seconds))
                .done {
                    seal.reject(ThreemaProtocolError.notLoggedIn)
                }
                .catch { error in
                    seal.reject(error)
                }
    ...
    private func showProgressUI() -> Promise<Void> {
        firstly { () -> Guarantee<ProgressViewController?> in
            ...
            self.progressViewController?.modalTransitionStyle = .crossDissolve

            return .value(self.progressViewController)
        }
        .then(on: .main) { progressController -> Promise<Void> in
            ...
            return self.itemSender.itemCount().then { itemCount in
                ...
                return Promise<Void> { seal in
                    DispatchQueue.main.async {
                        self.present(progressController, animated: true) {
                            _ = self.connectToServer().done(on: .main) {
                                // Send all items asynchronously
                                self.sendItems()
    ...
    private func startSending() {
        itemSender.itemCount().then(on: .main) { itemCount -> Promise<Bool> in
            ...
            return .value(true)
        }.then(on: .main) { doSending -> Promise<Void> in
            guard doSending else {
                return .value
            }

            // Show progress, connect to server and send items
            return self.showProgressUI()
        }.catch(on: .main) {
            DDLogError("Start sending failed: \($0)")

            if let error = $0 as? ThreemaProtocolError, case error = .notLoggedIn {
                // Dismiss progress and show connection error
...
extension ShareSheetRootNavigationController: ContactGroupPickerDelegate {
...
extension ShareSheetRootNavigationController: JKLLockScreenViewControllerDelegate {
...
extension ShareSheetRootNavigationController: ProgressViewDelegate {
...
extension ShareSheetRootNavigationController: ModalNavigationControllerDelegate {
...
extension ShareSheetRootNavigationController: SenderItemDelegate {
...
extension ShareSheetRootNavigationController: ConnectionStateDelegate {
    func changed(connectionState state: ConnectionState) {
        if state == .loggedIn {
            connectionStateResolver?.fulfill_()
            connectionStateResolver = nil
        }
        else if state == .disconnected {
            connectionStateResolver?.reject(ThreemaProtocolError.notLoggedIn)
            connectionStateResolver = nil
        }
    }
}
